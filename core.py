...
from file_utils import escape_path, moveFailedFolder, create_folder, get_info
from downloader import (
    download_file_with_filename, trailer_download, actor_photo_download,
    extrafanart_download, image_download, image_ext
)
from metadata import small_cover_check, print_files
...
def paste_file_to_folder_mode2(
    filepath, path, multi_part, number, part, leak_word, c_word, hack_word
):  # 文件路径，番号，后缀，要移动至的位置
    if multi_part == 1:
...
def core_main(movie_path, number_th, oCC, specified_source=None, specified_url=None):
    ...
    if conf.main_mode() == 1:
        ...
        if conf.is_watermark():
            add_mark(
                os.path.join(path, poster_path),
                os.path.join(path, thumb_path),
                cn_sub,
                leak,
                uncensored,
                hack,
        ...
        print_files(
            ...
            leak_word,
            c_word,
            json_data.get("naming_rule"),
            part,
            cn_sub,
            json_data,
            movie_path,
            tag,
            json_data.get("actor_list"),
            liuchu,
            uncensored,
            hack,
            hack_word,
