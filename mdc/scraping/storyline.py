...
def getStoryline(
    ...
    if uncensored:
        ...
        storyine_sites = (
            config.getInstance().storyline_censored_site().split(",") + storyine_sites
        )  # "2:airav,5:xcity".split(',')
    r_dup = set()
    sort_sites = []
    ...
    try:
        url = "http://58avgo.com/cn/index.aspx" + secrets.choice(
            [
                ...
                "?status=11",
                "?status=12",
                "?status=1&Sort=Playon",
                "?status=1&Sort=dateupload",
                "status=1&Sort=dateproduce",
            ]
        )  # 随机选一个，避免网站httpd日志中单个ip的请求太过单一
        kwd = number[:6] if re.match(r"\d{6}[\-_]\d{2,3}", number) else number
        result, browser = get_html_by_form(
            url,
