...
- **三种模式**：
  - 模式 1：刮削模式（移动文件）
  - 模式 2：整理模式（仅整理）
  - 模式 3：分析文件夹模式（不移动文件，可下载字幕并更新元数据；`-N` 无网络模式下不下载字幕）
  - 模式 4：映射整理模式（扫描含 NFO 的影片文件夹，按 mapping_actor/mapping_info 规范化 NFO 中演员与标签，并将影片文件夹移动到规范化后的演员目录）
- **链接支持**：支持软链接和硬链接方式
...
4. **模式选择**：
   - 模式 1：适合首次整理，会移动文件
   - 模式 2：适合重新整理已有文件
   - 模式 3：适合更新元数据，不移动文件（也会尝试下载字幕；`-N` 下仅本地处理）
